...
大多数平台无需登录态即可解析，如需按需增强以下特定功能，可通过管理员后台「系统设置 - 平台凭据」直接配置（即改即生效），或复制环境变量示例文件：
...
1. **小红书笔记与图集 (`XHS_COOKIE`)**：
   * 用于解决小红书网页端针对服务器 IP 匿名抓取的风控拦截（避免 302 重定向至登录页）。推荐精简 Cookie 配置 `a1=xxx; webId=xxx; web_session=xxx`。配置后解析成功率可达 99%+。
2. **抖音放映厅长视频 (`DOUYIN_COOKIE`)**：
...
通过 Docker Compose 快捷构建并启动服务；如需配置 Cookie，请先按上方说明配置 `.env`（或在启动后登录管理员后台配置）：
