...
说明：带 `*` 的项表示该素材目前仍保留官方原生水印，未带 `*` 的项均为无水印素材。
...
绝大多数平台**无需任何 Cookie 即可直接解析**。如需增强特定平台（如小红书防风控、豆包无水印视频、微信视频号高清流等），直接复制环境变量文件并在 `.env` 中按需填写即可：
...
* **仅限公开 UGC 内容**：仅解析普通用户公开发布的短视频、图集素材，**不支持**任何私密、好友可见等未公开内容。
* **拒绝 VIP / 付费破解**：**不解析**任何会员专享、单点付费、付费短剧等收费内容；系统不依赖亦不索取任何付费/VIP Cookie。
* **不支持版权长视频**：专注于短视频生态，**坚决不做**任何爱优腾芒、Netflix、电影、电视剧等长视频版权内容的嗅探与破解。
* **纯粹协议逆向，无破坏性**：不破坏 DRM 加密流，不从事任何黑产或恶意绕过行为。如遇平台接口调整或合规要求，将主动调整或下线对应解析模块。
